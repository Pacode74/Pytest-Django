...
class EmailUnitTest(TestCase):
    ...
    def test_send_email_without_arguments_should_send_empty_email(self)-> None:
        client = Client()  # (5.1)
        with patch("api.coronavstech.companies.views.send_mail") as mocked_send_mail_function:  # (6.1)
            ...
            self.assertEqual(response_content["info"], "email sent successfully")  # (5.6)
